...
| 5 | 은행 정상 안내 | SAFE |
| 6 | 보험 사기 | WARNING |
| 7 | 실제 목소리 보이스피싱 | WARNING |

### 4. 시나리오 분석

각 카드의 **분석** 버튼 클릭:
1. 실제 오디오가 **스피커로 재생** (약 10~25초)
2. **파형 애니메이션** 바가 움직임
3. 재생 중 **실시간 STT 전사 카드**가 오디오 진행률에 맞춰 표시됨
4. 재생 중 프로그레스 바 채워짐
5. 재생 완료 후 결과 카드가 뜸:
...
- [ ] #7: WARNING, 실제 목소리 + 피싱 키워드 "수사관", "계좌이체", "범죄", "수사", "오늘 안에" 등
...
- **소요 시간**: 7개 전체 ~3분 내외 (각 시나리오 재생 15~25초 + 설명 15초).
