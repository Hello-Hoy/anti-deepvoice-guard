...
- KMeans centroid 정규화 후 **전체 샘플 윈도우 중 top-min(200,n) 고유사 윈도우 평균**으로 1회 반복정제 → 화자에 더 타이트한 `anchor` (diarize_mp3_v2 검증 패턴). `_anchor/anchor.npy`로 저장하여 pilot/full에서 재사용
- 정제된 anchor에 가장 가까운, 서로 다른 3개 파일에서 대표 클립(각 ~10s) 추출 → `_anchor_candidates/`
- **사용자가 3개 클립을 듣고 "전현무 맞음" 확인** (확인 게이트). 부정 시 `--rebootstrap --k <다른값>`으로 재시도
