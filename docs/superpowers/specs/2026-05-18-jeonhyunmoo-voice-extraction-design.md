...
## 6. Anchor 부트스트랩 (전현무 식별 — 사용자 선택형)

**설계 배경:** 예능 짧은 클립은 나레이션 성우가 거의 모든 영상에 깔려, "최다 파일 커버리지 클러스터 자동 단일 선택"은 전현무가 아닌 성우를 잡는다(실측: 27/30 커버 클러스터가 성우). 도구가 하나를 찍어 맞히는 대신 **상위 클러스터 여러 개의 샘플을 만들어 사용자가 전현무를 직접 고르게** 한다.

- 짧은 클립 중 `sample_n`(기본 30)개 샘플 → 슬라이딩 윈도우 임베딩 수집
- KMeans(k 기본 12 — 성우/MC/게스트/음악 분리에 충분히 미세) 클러스터링
- 클러스터를 (서로 다른 소스 파일 수, 윈도우 수) 내림차순 정렬, 상위 `top_n`(기본 6)개 선정
- 각 상위 클러스터: 클러스터 centroid → top-min(200,n) 고유사 윈도우 평균으로 1회 반복정제(diarize_mp3_v2 패턴) → `_anchor/cluster_NN.npy` 저장; 서로 다른 3개 파일 대표 클립(각 ~8s)을 `_anchor_candidates/cluster_NN/`에 기록; 파일 커버리지·윈도우 수 진단 출력
- **사용자가 각 `cluster_NN` 폴더를 듣고 전현무인 클러스터 번호 N을 식별** (확인 게이트)
- `--use-cluster N` → `cluster_NN.npy`를 정식 `_anchor/anchor.npy`로 채택 후 pilot/full 진행. 적합 클러스터가 없으면 `--rebootstrap --k <다른값>`
